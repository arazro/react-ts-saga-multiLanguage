...
interface IjsonPlayer {
    ...
    | "loading2" | "location_pin" | "everyting" | "dashboard" | "" | "",
    height: number,
    width:number
};
...
let everyting: JSON = require('./jsons/everyting.json');
let dashboard: JSON = require('./jsons/dashboard.json');
...
const JsonPlayer: FC<IjsonPlayer> = ({ loading, loop = true, autoplay = true, jsonName,height,width }) => {
    ...
    if (jsonName === "everyting") AnimationData = everyting;
    if (jsonName === "dashboard") AnimationData = dashboard;
